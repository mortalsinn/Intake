...
function buildLeadRecord(lead, formConfig) {
    ...
    for (const field of formConfig.fields) {
        ...
        if (field.zoho) {
            ...
            // crmLabel lets a friendly booth question ("Is this for…") read as
            // a clean CRM line ("Project"). The screen and the record have
            // different audiences.
            const label = field.crmLabel || field.label;
            extras.push(`${label}: ${Array.isArray(value) ? value.join(', ') : String(value).trim()}`);
        }
    }
