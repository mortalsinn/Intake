...
const LEAD_SCOPE = 'ZohoCRM.modules.leads.CREATE';
const NOTE_SCOPE = 'ZohoCRM.modules.notes.CREATE';
const REQUIRED_SCOPE = `${LEAD_SCOPE},${NOTE_SCOPE}`;
...
function buildLeadRecord(lead, formConfig, { withDetail = false } = {}) {
    ...
    // Without this, Zoho applies the org's default status, which for this org
    // is "Contact in Future" — a fresh booth lead filed as deferred follow-up.
    ...
    if (withDetail) {
        ...
        if (lead.fields._boothRating) extras.unshift(`Booth assessment: ${lead.fields._boothRating}`);
        record.Description = [
            `Captured at ${formConfig.show.name} — ${stamp(lead.submittedAt, formConfig.show.timeZone)}`,
            ...extras,
            consent === null ? null : consent ? 'Consent: agreed to be contacted' : 'Consent: NOT given',
        ].filter(Boolean).join('\n');
    } else {
...
function buildNote(lead, formConfig) {
    ...
    const lines = [];
    lines.push(`${(show.name || 'HOME SHOW').toUpperCase()} — BOOTH INTAKE`);
    lines.push(when);

    if (lead.fields._boothRating) {
        lines.push('', 'BOOTH ASSESSMENT', `  ${lead.fields._boothRating}`);
    }
    if (contact.length) lines.push('', 'CONTACT', block(contact));
    if (project.length) lines.push('', 'WHAT THEY WANT', block(project));
    if (freeText) lines.push('', 'FROM THE CONVERSATION', `  ${freeText.value}`);
    if (consent !== null) {
        lines.push('', 'CONSENT', consent
            ? `  Agreed to be contacted — given at the booth, ${when}.`
            : '  NOT GIVEN — do not contact.');
    }
    lines.push('', '—', `Captured on the ${show.name} intake iPad.`);

    return {
        Note_Title: `Booth intake — ${show.name}`,
...
const DROPPABLE_PICKLISTS = new Set(['Lead_Status', 'Lead_Source']);
...
module.exports = {
    REQUIRED_SCOPE, LEAD_SCOPE, NOTE_SCOPE, DATACENTERS,
