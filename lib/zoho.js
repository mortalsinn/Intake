    ...
    if (withDetail) {
        ...
        record.Description = [
            ...
            consent === null ? null : consent
                ? 'Consent: given for contact regarding this enquiry'
                : 'Consent: NOT GIVEN — do not contact',
        ].filter(Boolean).join('\n');
    } else {
...
function buildNote(lead, formConfig) {
    ...
    const lines = [];
    lines.push(`${(show.name || 'HOME SHOW').toUpperCase()} — PROJECT ENQUIRY`);
    lines.push(when);

    if (lead.fields._boothRating) {
        lines.push('', 'FOLLOW-UP', `  ${lead.fields._boothRating}`);
    }
    ...
    if (consent !== null) {
        lines.push('', 'CONSENT', consent
            ? `  Consent given for contact regarding this enquiry, ${when}.`
            : '  CONSENT NOT GIVEN — do not contact.');
    }
