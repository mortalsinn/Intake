...
// How long a new lead is held back from Zoho and email so the staff
// priority tap can join it — a CEILING, not the usual wait.
//
// It used to be 20 seconds, which assumed the priority screen came straight
// after Submit. It does not: the visitor first gets the photograph QR (up to
// five minutes) and the thank-you (up to two), and only then does the iPad
// come back to staff. Nearly every lead went out before anyone could rate
// it, and the priority never reached the CRM or the email.
//
// The kiosk now releases the lead the moment the staff step ends — a tap,
// Skip, or any screen timing out — so a lead normally waits only as long as
// that takes. This ceiling covers an iPad that goes offline or is closed
// mid-flow. It must exceed the longest flow the kiosk can run (QR + thank
// you + staff step, in public/app.js); a test holds the two together. The
// lead is on disk throughout, so waiting costs nothing.
const HOLD_MS = 8 * 60 * 1000;

// Set DEMO_MODE=1 to build a kiosk that can never reach a CRM: for handing
// to somebody to try, or for a dry run before a show. Everything else about
// the app is unchanged, so what a tester exercises is the real pipeline.
const DEMO_MODE = /^(1|true|yes)$/i.test(process.env.DEMO_MODE || '');

// Refused for the sending account's setup (a bad key, an unverified domain),
// not for anything in the lead. The error text covers refusals recorded
// before the flag existed, which would otherwise wait for a manual Retry.
const isSetupRefusal = (m) => m?.status === 'failed' && (m.setup || /\((401|403)\)/.test(m.error || ''));
...
function createStore(dataDir) {
    ...
    return {
        ...
        /**
         * Start a new show with an empty lead log — by ARCHIVING, never deleting.
         *
         * The working file, the append-only journal and any held photographs
         * move together into data/archive/<timestamp>/, so the admin page
         * starts at zero and "Check nothing lost" starts clean, while every
         * past capture is still on disk if anybody ever needs it. The Zoho
         * connection stays where it is. Nothing is sent anywhere by this.
         */
        archiveAll() {
            const stamp = new Date().toISOString().replace(/[:.]/g, '-');
            const dest = path.join(dataDir, 'archive', stamp);
            fs.mkdirSync(dest, { recursive: true });
            const summary = {
                archivedTo: path.relative(dataDir, dest),
                leads: leads.length,
                undelivered: leads.filter(l => (l.zoho?.status === 'pending' || l.zoho?.status === 'failed')
                    || ((l.kind === 'ribit') && l.mail?.status !== 'sent')).length,
            };
            for (const f of [leadsFile, journalFile, photoDir]) {
        ...
        retry(id) {
            ...
            for (const lead of leads) {
                // 'local' is a draw entry: it is not incomplete, it is finished
                // and deliberately not in the CRM. Re-queueing it just counted
                // draw entries in "Re-queued N items" and sent them round the
                // pusher to be marked local again.
                // A synced lead whose note or tag failed is incomplete too: it
                // is in the CRM, but not filed where anybody will find it.
                const incomplete = (lead.zoho.status !== 'synced' && lead.zoho.status !== 'local')
                    || lead.zoho.noteError || lead.zoho.tagError;
                if (incomplete && (!id || lead.id === id)) {
                    lead.zoho.status = 'pending';
                    lead.zoho.error = undefined;
                    lead.zoho.noteError = undefined;
                    lead.zoho.tagError = undefined;
            ...
            // Email that failed goes back in the queue as well.
            for (const lead of leads) {
                if (id && lead.id !== id) continue;
                if (lead.mail?.status === 'failed') {
                    lead.mail = { status: 'pending', attempts: 0 };
                    n++;
                }
            }
            if (n) persist();
        ...
        /**
         * Leads waiting to be emailed, past their priority-tap grace period.
         *
         * Includes a lead refused for a SETUP reason (bad key, unverified
         * sending domain). It stays 'failed' so the booth sees the refusal,
         * but it is not the lead's fault, so it keeps being offered on the
         * backoff clock and goes by itself once the setup is fixed.
         */
        pendingMail: (kinds) => leads.filter(l =>
            kinds.has(l.kind || 'enquiry')
            && ((l.mail?.status || 'pending') === 'pending' || isSetupRefusal(l.mail))
            && !(l.holdUntil && Date.now() < l.holdUntil)),

        /** Send setup-refused email again now, rather than at the next backoff. */
        requeueSetupMail(kinds) {
            let n = 0;
            for (const l of leads) {
                if (!kinds.has(l.kind || 'enquiry') || !isSetupRefusal(l.mail)) continue;
                l.mail = { ...l.mail, attempts: 0, lastTriedAt: undefined };
                n++;
            }
            if (n) persist();
            return n;
        ...
        /** The most recent email refusal, in Resend's own words. */
        lastMailError(kinds) {
            const failed = leads.filter(l => kinds.has(l.kind || 'enquiry') && l.mail?.status === 'failed' && l.mail?.error)
                .sort((a, b) => String(b.mail.lastTriedAt || '').localeCompare(String(a.mail.lastTriedAt || '')));
            return failed[0]?.mail.error || null;
        },

        updateMail(id, patch) {
            const lead = leads.find(l => l.id === id);
            if (!lead) return null;
            lead.mail = { ...(lead.mail || { status: 'pending', attempts: 0 }), ...patch };
            persist();
            return lead;
        },
        ...
        mailCounts(kinds) {
            ...
            for (const l of leads) {
                if (!kinds.has(l.kind || 'enquiry')) continue;
                const st = l.mail?.status || 'pending';
                c[st] = (c[st] || 0) + 1;
            }
            return c;
        },
        ...
        counts() {
            ...
            // Per-kind, so the booth can see at a glance that BOTH brands are
            // landing. One combined total hides a brand that has stopped
            // syncing behind the other brand's healthy number.
            c.byKind = {};
            for (const l of leads) {
                const k = l.kind || 'enquiry';
                const b = c.byKind[k] || (c.byKind[k] = { total: 0, synced: 0, pending: 0, failed: 0, local: 0 });
                b.total++;
                b[l.zoho.status] = (b[l.zoho.status] || 0) + 1;
            }
            return c;
        },
        ...
        getZoho: () => {
            // Demo mode: the single chokepoint every Zoho call passes through.
            // A tester running the container must not be able to write into a
            // live CRM, and the safe way to guarantee that is to refuse to
            // HAND OUT the credentials rather than to remember to check a flag
            // at each of the call sites. Leads still queue, persist and export
            // exactly as they do at a real booth — they just never leave.
        ...
        setZoho(cfg) {
            if (DEMO_MODE) throw new Error('Demo mode: this build cannot connect to Zoho CRM.');
...
module.exports = { createStore, HOLD_MS };
