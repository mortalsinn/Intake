...
function buildNote(lead, formConfig) {
    ...
    const lines = [];
    // The brand owns this wording. A Code Compass demo request filed under
    // the heading "PROJECT ENQUIRY" reads like a railing job to whoever
    // picks it up.
    lines.push(`${(show.name || 'Fall Home Show 2026').toUpperCase()} — ${show.enquiryLabel || 'PROJECT ENQUIRY'}`);
    lines.push(when);
    ...
    if (project.length) lines.push('', show.detailLabel || 'PROJECT REQUIREMENTS', block(project));
    ...
    lines.push('', '—', `Recorded at the ${show.name} via the ${show.appName || 'Ironwood intake application'}.`);

    return {
        Note_Title: `${show.noteTitle || 'Project enquiry'} — ${show.name}`,
...
/**
 * Tag a lead, so a show's leads can be found by searching rather than by
 * parking them in a status column of their own.
 *
 * add_tags creates a tag that does not exist yet, and needs only the
 * module scope this app already holds — verified against the live CRM,
 * including reading the tag back off the lead. A tag that fails never
 * costs the lead: the caller records the failure and Retry re-applies it.
 */
async function addTags(cfg, leadId, names) {
    const tags = [].concat(names || []).filter(Boolean).map(name => ({ name: String(name) }));
    if (!tags.length) return;
    const token = await accessToken(cfg);
    const res = await fetch(`${apiBase(cfg.datacenter)}/Leads/actions/add_tags`, {
        method: 'POST',
        headers: { Authorization: `Zoho-oauthtoken ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ tags, ids: [String(leadId)] }),
    });
    const body = await res.json().catch(() => ({}));
    const result = body.data?.[0];
    // Success is what Zoho says about THIS record, not the HTTP status: a
    // bulk endpoint answers 200 with a per-record failure inside it.
    if (!res.ok || result?.status !== 'success') {
        throw new Error(`Tag refused: ${result?.code || body.code || `HTTP ${res.status}`} ${result?.message || body.message || ''}`.trim());
    }
}
...
module.exports = {
    ...
    exchangeCode, accessToken, buildLeadRecord, buildNote, createLead, createNote, addTags,
